...
cConnectionSocket = socket(AF_INET, SOCK_STREAM)

#Attempt connection to specified server on specified port
cConnectionSocket.connect((inputParameters["serverName"], inputParameters["serverPort"]))

#Send command string to server
cConnectionSocket.send((inputParameters["fileName"] + dataPortMessage + 
	inputParameters["command"][1]).encode())
	
#Receive acknowledgment from server on control connection
connectionResponse = cConnectionSocket.recv(2)
#print(connectionResponse.decode("utf-8", "ignore") + "\n")

#If command was accepted, establish a data transfer connection 
#socket on specified data port number, and listen for incoming data
if connectionResponse.decode("utf-8", "ignore") == ("CA"):
	dtSocket = socket(AF_INET, SOCK_STREAM)
	#Bind the port number to the socket
	socketUnbound = True
	while(socketUnbound):
		try:
			dtSocket.bind(('', inputParameters["dataPort"]))
			socketUnbound = False
		except:
			print("\nPort " + str(serverPort) + " unavailable. Select a different" 
				" port for data transfer.")
			inputParameters["dataPort"] = int(input("\nEnter Data Port Number\n--> "))
			while not(int(inputParameters["dataPort"]) >= 3000 
						and int(inputParameters["dataPort"]) <= 65535):
				print("Data Port Number must be an integer in range of 3000 to 65,535.")
				inputParameters["dataPort"] = int(input("Enter Server Port Number\n--> "))
	#Listen on the Socket (max number of queued connections is 1).
	dtSocket.listen(1)
	
	#Receive acknowledgment from server on control connection that data port
	#--is available on server
	connectionResponse = cConnectionSocket.recv(2)
	
	if connectionResponse.decode("utf-8", "ignore") == ("PA"):
		#Create a new socket for connection once sender initiates contact
		dtConnectionSocket, addr = dtSocket.accept()
		#if -l command entered, send directory structure
		if inputParameters["command"] == "-l" or inputParameters["command"] == "-L":
			dataTransferResponse_Directory = dtConnectionSocket.recv(8192)
			print("\nReceiving directory structure from " + inputParameters["serverName"] + 
					" on port " + str(inputParameters["dataPort"]) + "\n", 
			...
			incomingFileSize = dtConnectionSocket.recv(32).decode("utf-8", "ignore")
			...
			dtConnectionSocket.send("File Size Received".encode())
			
			print('\nReceiving "' + inputParameters["fileName"] + '" from ' +
					inputParameters["serverName"] + " on port " + 
					str(inputParameters["dataPort"]) + "\n")
			
			#Receive data blocks, write to file, and acknowledge each block
			while numFullChunks > 0:
				dataTransferResponse_FileContents = dtConnectionSocket.recv(512)
				f.write(dataTransferResponse_FileContents)
				dtConnectionSocket.send(("Chunk Received").encode())
				numFullChunks -= 1	
			dataTransferResponse_FileContents = dtConnectionSocket.recv(int(remainder))
			f.write(dataTransferResponse_FileContents)
			dtConnectionSocket.send(("Chunk Received").encode())
			#Send final acknowledgment to server
			dtConnectionSocket.send(("File Received").encode())
		...
		#Close data transfer connection
		dtConnectionSocket.close()
		
	else:
		print("\n" + inputParameters["serverName"] + ":" + 
			str(inputParameters["serverPort"]) + " says PORT " +
			str(inputParameters["dataPort"]) + " unavailable (" +
...
elif connectionResponse.decode("utf-8", "ignore") == ("IV"):
